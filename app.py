...
def FUN_private():
    if "current_user" in session.keys():
        ...
        notes_table = zip([x[0] for x in notes_list], [x[1] for x in notes_list], [x[2] for x in notes_list],
                          ["/delete_note/" + x[0] for x in notes_list])

        images_list = list_images_for_user(session['current_user'])
        images_table = zip([x[0] for x in images_list], [x[1] for x in images_list], [x[2] for x in images_list],
...
def FUN_admin():
    if session.get("current_user", None) == "ADMIN":
        ...
        user_table = zip(range(1, len(user_list) + 1), user_list,
...
# Reference: http://flask.pocoo.org/docs/0.12/patterns/file
# ads/
...
def FUN_login():
    ...
    if (id_submitted in database.list_users()) and database.verify(id_submitted, request.form.get("pw")):
...
def FUN_delete_user(id):
    if session.get("current_user", None) == "ADMIN":
        ...
        database.delete_user_from_db(id)
    ...
    if session.get("current_user", None) == "ADMIN":  # only Admin should be able to add user.
        ...
        if " " in request.form.get('id') or "'" in request.form.get('id'):
            ...
            user_table = zip(range(1, len(user_list) + 1),
                             user_list, [x + y for x, y in zip(["/delete_user/"] * len(user_list), user_list)])
